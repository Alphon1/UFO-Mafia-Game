...
public class Player_Visionfortut : MonoBehaviour
{
    [SerializeField]
    ...
    {
        if (Enemy.GetComponent<Enemy_Managerfortut>())
        {
            ...
            if (!Physics.Linecast(Player_Character.transform.position, Enemy.transform.position, ~(1 << 8)))
            {
                ...
                if (!Seen_Enemies.Contains(Enemy.gameObject))
                {
                    ...
                    Enemy.GetComponent<Enemy_Managerfortut>().Seen_By += 1;
    ...
    private void OnTriggerExit(Collider Exiting_Enemy)
    {
        ...
        {
            Exiting_Enemy.GetComponent<Enemy_Managerfortut>().Seen_By -= 1;
